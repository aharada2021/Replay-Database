...
- 動画アップロードがリプレイ処理より後になる場合、`handle_complete_multipart`で自動マイグレーション
- 判定: `is_temp_arena_id()` - 16桁hex かつ 全数字でない
- 解決: `find_arena_unique_id_by_temp_id()` - s3Keyプレフィックス検索で正式ID取得
...
## 完了したタスク
- **ゲームプレイ動画S3キー マイグレーション修正（2026-02-01）**:
  - 問題: 動画アップロード完了がリプレイ処理完了より後になった場合、動画が一時IDパスに残り再生不可
  - 原因: クライアントは`tempArenaInfo.json`から生成したMD5ハッシュ（一時ID）を使用、リプレイ処理後に正式arenaUniqueIDが判明
  - 修正: `handle_complete_multipart`で一時IDを検出し、正式IDへ自動マイグレーション
  - `src/utils/dynamodb_tables.py`:
    - `is_temp_arena_id()`: 一時ID判定（16桁hex、全数字でない）
    - `find_arena_unique_id_by_temp_id()`: wows-replaysテーブルからs3Keyプレフィックス検索で正式ID取得
  - `src/handlers/api/upload_multipart.py`:
    - 一時ID検出時に正式arenaUniqueIDを検索
    - S3オブジェクトを正式パスにコピー＆旧パス削除
    - DynamoDB更新を正式IDで実行
  - CloudFront: `/gameplay-videos/*` ビヘイビア追加（S3オリジンへルーティング）
