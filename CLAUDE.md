...
- **Dockerfile統合: extractor/processor → 単一lambda（2026-03-11）**:
  - Rust移行で両Dockerfileの差分がCMD1行のみになったため統合
  - `deploy/Dockerfile.lambda`: 統合Dockerfile（CMDは`serverless.yml`の`image.command`で上書き）
  - `deploy/serverless.yml`: 全関数が`ECR_IMAGE_URI`を参照（旧`ECR_EXTRACTOR_IMAGE_URI`/`ECR_PROCESSOR_IMAGE_URI`廃止）
  - `.github/workflows/deploy-lambda.yml`: ビルドジョブ統合（2→1）、`workflow_dispatch`のdev deploy修正
  - 効果: CI Docker ビルド時間半減、ECRストレージ削減
- **Rust移行 Phase 4: 旧Pythonパーサー/サブモジュール完全除去（2026-03-11）**:
  ...
  - 旧Dockerfile削除: `deploy/Dockerfile`（Dockerfile.processor/extractor に置換済み）
  - `battle_result_extractor.py`: Python fallback除去、Rust常時使用化
  - `Dockerfile.extractor`: replays_unpack/ships.json/parsers参照除去
  - `requirements_lambda.txt`: pycryptodomex/packaging除去
  - `serverless.yml`: USE_RUST_EXTRACTOR環境変数除去
  - `captain_skills.py`: ships.json依存除去
...
- **Rust抽出パイプライン移行 Phase 2（2026-03-11）**:
  ...
  - `deploy/serverless.yml`: `USE_RUST_EXTRACTOR`/`GAME_DATA_DIR` 環境変数追加（dev: true, prod: false）
  - `.github/workflows/deploy-lambda.yml`: `cargo-zigbuild` でGLIBC 2.26ターゲット（Lambda AL2互換）
  - `rust/wows-replay-tool/src/extract.rs`: wows-toolkit翻訳API使用（艦艇名、スキル、アップグレード、消耗品、信号旗、マップ名）
  - `src/handlers/processing/battle_result_extractor.py`: Rust出力からmapDisplayName取得
  - `src/utils/dynamodb_tables.py`: GAME_TYPE_MAP二重正規化バグ修正（`"random": "random"` 追加）
  - `scripts/generate_ja_mo.py`: 既存Pythonマッピングから日本語MOファイル生成（スキル71、アップグレード118、マップ42件）
  - S3に日本語MOアップロード済み: `game-data/{version}/translations/ja/LC_MESSAGES/global.mo`
  - 翻訳言語: `WOWS_LANG` 環境変数（デフォルト `ja`、フォールバック `en`）
...
## 今後の予定
- **Python 3.10 → 3.12 アップグレード**: minimap_renderer除去によりPython 3.10制約が解消
