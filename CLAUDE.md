...
- **ゲームプレイ動画キャプチャ バグ修正（2026-02-01）**:
  - 音声ノイズ問題修正: `audio_capture.py`からリサンプリングコードを完全に削除
    - `_resample_audio()` 関数削除
    - `_mix_audio()` からリサンプリング呼び出し削除
    - サンプルレート不一致時はマイク音声を破棄（ノイズ防止）
  - 動画フレームタイミング修正: `video_encoder.py`にフレーム補間を追加
    - `-vf fps=fps=30:round=near` でフレーム補間
    - `-vsync cfr` で固定フレームレート出力
  - 音声・映像同期修正: FFmpeg muxに同期オプション追加
    - `-async 1` と `-af aresample=async=1000` で音ズレ補正
  - 動画アップロード失敗修正: S3マルチパートアップロード実装
    ...
    - `client_tool/wows_replay_uploader.py`: マルチパートアップロード対応
      ...
      - `_upload_gameplay_video_multipart()`: マルチパートアップロード実装
    - `deploy/serverless.yml`: 新規エンドポイント追加
