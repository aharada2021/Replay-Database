        ...
        if ac := self._renderer.conman.active_consumables.get(
            ...
            if c_image := self._consumable_cache.get(aid_hash, None):
                ...
                for aid, _ in ac.items():
                    ...
                    # Try both string and integer keys for JSON compatibility
                    aid_str = str(aid)
                    try:
                        index = abilities["id_to_index"].get(aid_str) or abilities["id_to_index"].get(aid)
                        if index is None:
                            raise KeyError(aid)
                    except KeyError:
                        try:
                            index = self._abilities["clan"].get(aid_str) or self._abilities["clan"].get(aid)
                            if index is None:
                                # Skip unknown ability IDs
                                continue
                        except KeyError:
                            continue
